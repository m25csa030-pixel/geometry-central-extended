...
* **Phase 1 (Standalone CUDA PCG Solver)**: Complete (`gpu_solver/`). Validated with 13/13 unit tests.
* **Phase 2 (Geometry-Central Library Integration)**: Complete (`include/geometrycentral/numerical/cuda_pcg_solver.h`, `src/numerical/cuda_pcg_solver.cu`). Opt-in via `GC_ENABLE_CUDA=ON`.
* **Phase 3 (Heat Method Integration)**: Complete (`include/geometrycentral/surface/heat_method_distance.h`, `src/surface/heat_method_distance.cpp`). `HeatSolverBackend::CUDA_PCG` fully tested across resolutions with GoogleTest and benchmarks.
* **Phase 4 (Upstream Verification)**: Complete. 206/206 GoogleTest tests passing.
* **Next Step**: Complex Hermitian PCG for Vector Heat Method (`VectorHeatMethodSolver`).
